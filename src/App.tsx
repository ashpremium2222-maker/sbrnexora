...
type Expense = {
  id: string; tripId?: string; category: "Fuel" | "Toll" | "Maintenance" | "Salary" | "Allowance" | "Parking" | "Other";
...
const NAV_ITEMS: NavItem[] = [
  ...
  { view: "balanceFreight", label: "Vehicle Register", icon: ClipboardList, section: "Finance" },
    ...
    <div className="min-h-screen w-full flex font-[Inter,sans-serif] relative overflow-hidden" style={{ background: "var(--background)" }}>
      <aside className={`hidden md:flex flex-col py-5 px-2 gap-0.5 z-20 relative overflow-y-auto transition-all bg-[#0B111C] shadow-2xl ${collapsed ? "w-[88px] items-center" : "w-[238px]"}`}>
        <div className={`mb-3 flex items-center gap-3 ${collapsed ? "justify-center" : "justify-between w-full"}`}>
          <button onClick={() => setView("dashboard")} className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white shadow-lg shrink-0"><Truck size={20} color="#2563EB" /></button>
...
export default function App() {
  ...
  const [notes, setNotes] = useState(() => generateNotifications(seedVehicles, seedDrivers, seedInvoices));
  ...
  return (
    <Shell view={view} setView={setView} role={role} logout={() => { setRole(null); setAuthToken(null); localStorage.removeItem("sbr-role"); localStorage.removeItem("sbr-token"); }} unread={unread} profileName={profileName} theme={theme} toggleTheme={toggleTheme}>
      ...
      {modal === "expense" && <Modal title="Add Expense" onClose={() => setModal(null)}><ExpenseForm form={form} setForm={setForm} trips={trips} vehicles={vehicles} onSave={() => {
        const category = (form.category || "Other") as Expense["category"];
...
function Trips({ trips, customers, vehicles, drivers, role, search, setSearch, openModal, updateTripStatus, setView, exportCsv, onBill, edit, remove, onPodUpload }: { trips: Trip[]; customers: Customer[]; vehicles: Vehicle[]; drivers: Driver[]; role: Role; search: string; setSearch: (v: string) => void; openModal: (m: string) => void; updateTripStatus: (id: string, status: TripStatus) => void; setView: (v: View) => void; exportCsv: (n: string, rows: Record<string, string | number>[]) => void; onBill: (id: string) => void; edit: (trip: Trip) => void; remove: (id: string) => void; onPodUpload: (tripId: string, files: UploadedFile[]) => void }) {
  ...
  const filtered = relevant.filter((e) => (!dateFilter || e.date === dateFilter) && (tripFilter === "All" || e.tripId === tripFilter));
...
function MaintenanceModule({ records, vehicles, trips, openModal, exportCsv }: { records: MaintenanceRecord[]; vehicles: Vehicle[]; trips: Trip[]; openModal: (m: string, f?: Record<string, string>) => void; exportCsv: (n: string, rows: Record<string, string | number>[]) => void }) {
  const [dateFilter, setDateFilter] = useState("");
...
function AttendanceModule({ drivers, records, markAttendance, exportCsv, select }: { drivers: Driver[]; records: AttendanceRecord[]; markAttendance: (driverId: string, date: string, status: AttendanceRecord["status"], notes?: string) => void; exportCsv: (n: string, rows: Record<string, string | number>[]) => void; select: (id: string) => void }) {
  const [month, setMonth] = useState(today.slice(0, 7));
  const dates = Array.from({ length: daysInMonth(month) }, (_, index) => `${month}-${String(index + 1).padStart(2, "0")}`);
  const monthRows = records.filter((item) => item.month === month);
...
function TripForm({ form, setForm, customers, vehicles, onSave }: { form: Record<string, string>; setForm: React.Dispatch<React.SetStateAction<Record<string, string>>>; customers: Customer[]; vehicles: Vehicle[]; onSave: (files: string[]) => void }) {
  ...
  return <>
    <SelectField label="Category" value={form.category || "Other"} onChange={(v) => set("category", v)} options={["Fuel", "Toll", "Maintenance", "Salary", "Allowance", "Parking", "Other"].map((x) => ({ value: x, label: x }))} />
